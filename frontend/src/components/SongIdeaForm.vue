<!-- File: src/components/SongIdeaForm.vue -->
<template>
    <form @submit.prevent="submitForm" class="mb-8">
      <div class="mb-4">
        <label for="songIdea" class="block text-sm font-medium text-gray-700">Song Idea</label>
        <textarea
          id="songIdea"
          v-model="songIdea"
          required
      ...
      <div class="mb-4">
        <label for="numberOfDrafts" class="block text-sm font-medium text-gray-700">Number of Drafts</label>
        <select
          id="numberOfDrafts"
  ...
  export default {
    ...
    setup(props, { emit }) {
      ...
      const submitForm = () => {
        emit('generate-drafts', songIdea.value, numberOfDrafts.value);
      };
